...
const preprocessor = {
  ...
  execute : function (template, options, callback) {
    ...
    for (let i = -1; i < template.embeddings.length; i++) {
      let _mainOrEmbeddedTemplate = template.filename;
      let _parentFilter = '';
      let _fileType = template.extension;
      if (i > -1) {
  ...
  processQrcodes : function (template) {
    let documents = [
      template.files.find((x) => x.name === 'word/document.xml'),
    ...
    documents.forEach((document) => {
      xml2js.parseString(document.data, (err, result) => {
        ...
        const matches = xpath.find(result, '//pic:pic');

        // For each match
        matches.forEach((match) => {
          // Find the description tag
          let definition = xpath.find(xpath.find(match, '//pic:nvPicPr')[0], '//pic:cNvPr')[0].$;
      ...
      xml2js.parseString(document.data, (err, result) => {
        ...
        matches.forEach((match) => {
          ...
          if (fullUrl.includes('#contains')) {
            ...
            document.data = document.data.split('name="' + definition.name + '"').join('name="' + definition.name + '" contains="true" dynamic="true"');
          }
          else if (fullUrl.includes('{')) {
            document.data = document.data.split('name="' + definition.name + '"').join('name="' + definition.name + '" dynamic="true"');
          }
        });
      });
    });
  ...
  convertSharedStringToInlineString : function (template, parentFilter) {
    ...
    for (var f = 0; f < _filesToConvert.length; f++) {
      ...
      _modifiedFile.data = preprocessor.removeRowCounterInWorksheet(preprocessor.convertToInlineString(_modifiedFile.data, _sharedStrings));
    }
    return template;
  },
  ...
  convertNumberMarkersIntoNumericFormat : function (template) {
    ...
    if (_contentFileId > -1 && !!template.files[_contentFileId] === true) {
      template.files[_contentFileId].data = template.files[_contentFileId].data.replace(
