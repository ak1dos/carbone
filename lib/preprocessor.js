...
var preprocessor = {
  ...
  execute: function (template, options, callback) {
    ...
    for (let i = 0, len = template.files.length; i < len; i++) {
      template.files[i].data = preprocessor.preParseXML(
        template.files[i].data,
        options
      );
    }
  ...
  removeCalcChain: function (template, parentFilter) {
    ...
    for (var i = 0; i < template.files.length; i++) {
      var _file = template.files[i];
      if (
        _file.parent === parentFilter &&
        /xl\/calcChain\.xml$/.test(_file.name) === true
      ) {
        ...
        matches.map((match) => {
          ...
          if (fullUrl.includes("#contains")) {
            ...
            document.data = document.data
              .split('name="' + definition.name + '"')
              .join(
                'name="' + definition.name + '" contains="true" dynamic="true"'
              );
