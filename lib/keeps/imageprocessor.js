...
const imageprocessor = {
   ...
   * @returns {*}
   */
  transformContainedImage : function (document) {
    ...
    xml2js.parseString(document.data, (err, root) => {
      ...
      const matches = xpath.find(root, '//w:drawing').filter((x) => {
        const wp = x['wp:anchor'] || x['wp:inline'];
        return wp[0] !== '';
      });
      let state = [];
      matches.forEach(container => {
        const picture = xpath.find(container, '//pic:pic')[0];
        const definition = xpath.find(xpath.find(picture, '//pic:nvPicPr')[0], '//pic:cNvPr')[0].$;
        ...
        let spPr = xpath.find(picture, '//pic:spPr')[0];
        ...
        if (dimensions) {
          ...
          if (dimensions.width > dimensions.height) {
            ...
            const wp = container['wp:anchor'] || container['wp:inline'];
            wp[0]['wp:extent'][0] = { $ : size };
        ...
        xpath.find(container, '//pic:spPr')[0] = spPr;
      })
   ...
   */
  processDynamicImage : function (report, currentState, document, drawing, picture, contained, qrCodeSection, fullUrl, index) {
