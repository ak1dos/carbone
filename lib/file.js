...
async function zipFiles (filesToZip, callback) {
  let _previousParentName = null;
  let _index = filesToZip.length - 1;
  for (; _index >= 0; _index--) {
    const _file = filesToZip[_index];
    ...
    if (_file.data.includes('<office:binary-data>')) {
      ...
      while ((match = rx.exec(_file.data)) !== null) {
        ...
        // Replace qrcode url by a base64 binary
          ...
          let result = await _generateQR(url);
  ...
  const _groupOfFileToZip = filesToZip.splice(_index + 1);
