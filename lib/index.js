...
// Local imports
const locales = require('../formatters/_locale');
...
const postprocessor = require('./keeps/postprocessor');
...
const carbone = {
  ...
  reset : function () {
    ...
    const _tmpDir = parseInt(_nodeVersion[0], 10) === 0 && parseInt(_nodeVersion[1], 10) < 10 ? os.tmpDir() : os.tmpdir();
  ...
  removeTemplate : function (fileId, callback) {
    const _fullPath = path.join(params.templatePath, fileId);
  ...
  listConversionFormats : function (documentType) {
    ...
    const _doc = format[documentType];
    for (const attr in _doc) {
      const _format = _doc[attr];
      _format.id = attr;
      _res.push(_format);
    }
    return _res;
  ...
  /**
   * Renders a template with given data's and return result to the callback function
   *
   * @param {String}       templatePath : file name of the template (or absolute path)
   * @param {Object|Array} data : Data's to be inserted in the template represented by the {d.****}
      ...
      file.openTemplate(templatePath, function (err, template) {
        ...
        preprocessor.execute(template, options, function (err, template) {
          ...
          walkFiles(template, data, options, 0, function (err, report) {
            ...
            // Post processing of the report (KEEPS)
            postprocessor.execute(report, function (err, report) {
              ...
              file.buildFile(report, function (err, result) {
                if (err) {
                ...
                else if (typeof options.convertTo === 'string' && options.convertTo !== '') {
                  ...
                  typeof options.convertTo.extension === 'string' &&
                  options.convertTo.extension !== ''
                ) {
                  convert(result, report.reportName, options, function (err, result) {
 ...
 * @param {Object}  options          options coming from input.parseOptions and input.parseConvertTo
 *                                     {
 *                                       convertTo : {
 *                                         extension  : 'pdf',
